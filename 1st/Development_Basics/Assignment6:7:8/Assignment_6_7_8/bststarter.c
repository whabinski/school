...
void simple_print(bstNode *node, int depth);                            //print tree
bool verifyBST(bstNode *node, int minkey, int maxkey);                  //verifies bst
bstNode* create_node(int new_key);                                      //creates a new node not inserted into the treee
bstNode *insert(bstNode *node, int insert_key, bool *was_inserted);     //inserts the node into the correct spot in tree
int find_minimum(bstNode *node);                                        //min num in tree
int find_maximum(bstNode *node);                                        //max num in tree
bool is_member(bstNode *node, int find_key);                            //returns true if num is in the tree
bstNode *delete(bstNode *node, int delete_key, bool *was_deleted);      //deletes a node in the tree

// Functions created during week 9 labs
void pre_order(bstNode *node);                                          //prints regular simple print
void in_order(bstNode *node);                                           //prints lowest - highest node
void reverse_in_order(bstNode *node);                                   //prints highest to lowest
void post_order(bstNode *node);                                         //prints decendents first from left
int num_nodes(bstNode *node);                                           //number of node in a tree
void breadth_first(bstNode *node);                                      //prints each depth from left to right
...
int *array_of_sorted_keys(bstNode *node){
  ...
  int length = num_nodes(node);

  int *array = calloc(length, sizeof(bstNode*));

  int index = 0;
  build_array(node, array, &index);
