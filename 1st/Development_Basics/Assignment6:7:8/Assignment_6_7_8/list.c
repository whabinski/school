...
// Adds the value of two nodes with the corresponding placement among the linked list together.
// Stores the result in list 1
// If list1 is shorter than list 2 or vice versa, then no or nodes past the shorter of the two 
// lists no addition needs to occur
void add_lists(Node *list1, Node *list2){

  Node *current1 = list1; // placeholder to traverse through list 1
  Node *current2 = list2; // placeholder to traverse through list 2

  if (current1 == NULL || current2 == NULL) return; // if either list is empty, then nothing happens

  while (current1 != NULL){ // traverses through the linked lists until list 1 is at the end

    if (current2 == NULL) break; // if list 2 ends before list 1, then it breaks the loop

    current1->value = current1->value + current2->value; // adding the two values
    current1 = current1->next; // moving to next node in list 1
    current2 = current2->next; // moving to next node in list 2

  }
  return;
}

// Duplicated all nodes in a linked list into a newly created linked list in the heap.
Node *duplicate_list(Node *head){

  Node *newList = NULL; // creates new linked list with 0 nodes
  Node *current = head; // placeholder to traverse through origional list

  if (current == NULL) return NULL; // if the origional list is empty, return an empty list

  while (current != NULL){ // loop to traverse through every node in the origional linked list

    newList = insert_at_tail(newList, current->value); // the new list adds a node with same value and same place as the duplicated list
    current = current->next; // moving to next node in origional list
  }

  return newList; // returns new linked list
}
